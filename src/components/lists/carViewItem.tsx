...
type CarViewItemType = {
    url: string,
    name: string,
    price: number,
    days: number
}
const CarViewItem = ({url, name, price, days}: CarViewItemType) => {

  return (
    <a href="">
        <div className='flex hover:bg-gray-100 transition-all cursor-pointer hover:shadow-lg rounded-xl overflow-hidden border'>
            <div className=" relative bg-gray-50 w-[391px] h-[176px]">
                <Image src={url} alt='' fill sizes="(min-width: 808px) 50vw, 100vw" 
                style={{
            ...
            <div className="w-full py-2 px-5">
                ...
                <div className="pt-3 flex justify-between flex-col h-[132px]">
                    ...
                    <span className='text-sm text-gray-800 block text-right'> ₦{days}/day <small className="block underline"> ₦{ new Intl.NumberFormat().format(Math.abs(days*price))} est. total</small></span>
                </div>
            </div>
        </div>
    </a>
